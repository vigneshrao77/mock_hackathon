...
import { connectToDatabase, isMongoConnected } from './db.ts';
...
// Fallback in-memory store when MongoDB connection string is not yet supplied
...
// Seed MongoDB if database collection is empty
...
async function ensureMongoSeeded() {
  ...
  try {
    ...
    if (count === 0) {
      ...
      console.log('🌱 Seeded demo accounts into MongoDB successfully.');
    }
    mongoSeeded = true;
  } catch (err) {
...
export async function handleAuthRequest(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  if (url === '/api/auth/signup' && req.method === 'POST') {
    try {
      ...
      // Try connecting to MongoDB if available
      ...
      // Fallback in-memory storage if MongoDB connection string not set yet
